...
<form method="post" action="<?= $url ?>"  v-on:submit.prevent="send">
	<div class="card-body">
		...
		<div class="row row_mb" >
			...
			<div class="col-md-12">
			<div class="input_label"><?= lang('aeronautical_english_level')?></div>
				
				<select name="lang_level" class="form-control selectpicker" >
					<?foreach($lang_level as $id=> $value):?>
					<option 
					<?= $aero_lang['lang_level'] && $aero_lang['lang_level'] == $id ? 'selected' : null ?>
